...
def Create(request: CreateClientModel, session: Session = Depends(get_db), current_user: UserModel = Depends(get_current_client)):
...
def Get_by_id(id: int, session: Session = Depends(get_db), current_user: UserModel = Depends(get_current_client)):
